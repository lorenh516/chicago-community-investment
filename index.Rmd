...
```{r facet_wrap_demo_info, message=FALSE, warning=FALSE, fig.width=30, fig.height=20}
...
ggplot() +
  ...
  geom_sf(aes(alpha=medianIncome, color=predominant_race), lwd = 2, fill="#318791") +
  # scale_alpha(range = c(0.15, 1)) +
  
  # outline Chicago wards over data
  geom_sf(data = wards.2015, color="black", lwd = 0.5, fill=NA) +
  scale_color_manual(values=c("Asian" = "#986769", 
                              "Black" = "#9d8e64", 
                              "Latinx" = "#6d7d53", 
                              "White"=NA)) +
  ...
  theme(
        ...
        plot.subtitle = element_text(margin = unit(c(0, 0, 1, 0), "lines"), size=rel(1.5)),
        plot.caption = element_text(hjust=1),
        strip.text.x = element_text(family="Ledger", size=rel(1.5)),
        legend.position = "top",
  ...
  facet_wrap(~ id) +
  labs(alpha = "Median Income", color="Predominant Race", title="Latinx Populations in Chicago Pushed to South, West Neighborhoods", caption="Data Source: U.S. Census Bureau", subtitle = "Chicago racial and ethnic group movement by census tract since 2012 (5 year averages)") 
