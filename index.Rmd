...
```{r facet_wrap_demo_info, message=FALSE, warning=FALSE, fig.width=30, fig.height=15}
...
ggplot() +
  ...
  geom_sf(aes(alpha=medianIncome), lwd = 0, fill=get_dt_cols("ocean")) +
  # scale_alpha(range = c(0.15, 1)) +
  geom_sf(data = ward_range %>% filter(predominant_race != "White"), aes(color=predominant_race), lwd = 2, fill=NA) +
  ...
  scale_color_dt("diverging", shift=1, reverse = TRUE, na.value=get_dt_cols("palegray")) +
  coord_sf(datum = NA) +
  theme_map_modest() + 
  theme(
        plot.title = element_text(face = "bold", hjust="0.5", family = "Ledger", size=rel(1.5)),
        plot.subtitle = element_text(margin = unit(c(0, 0, 1, 0), "lines"), size=rel(1.25)),
        plot.caption = element_text(hjust=1),
        strip.text.x = element_text(family="Ledger", size=rel(1.2)),
        legend.position = "top",
