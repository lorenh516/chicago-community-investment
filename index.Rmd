...
bus_licenses %>% filter(!is.na(WARD), active==1,  activity_date >= mdy("1/1/2012")) %>%
  ...
  labs(x="Chicago Council Ward", y="Average Monthly Business Count", caption="Data Source: Chicago Open Data Portal", title="Least New & Surviving Businesses in Far South, Southwest Wards for 5+ Years", subtitle="Since 2012, Far South wards average 26 monthly business license issuances and renewals or less",fill="Chicago Area") + 
  theme_modest() +
  theme(
        panel.grid.major.x = element_blank(),
        axis.ticks =  element_blank(),
        axis.title.x = element_blank(),
        axis.text.x = element_text( family="Ledger", size=rel(1.25)),
        axis.title.y = element_text(margin=unit(c(0,1,0,0), "lines"), size=rel(1.5), angle = 90, family = "Ledger"),
        plot.caption = element_text(hjust=1)
        ) +
  annotate("text", x = 7 , y= 370, label="Most West Side business\noccurs in Wards 2 and 27,\nwhich border the Loop.", size=rel(6)) +
  annotate("text", x = 9 , y= 735, label="The Central area is comprised\nof Ward 42, the Loop area.", size=rel(6)) +
  annotate("text", x = 5 , y= 315, label="South Side business activity\nis concentrated in Kenwood,\n Fuller Park areas bordering\nthe University of Chicago.", size=rel(6)) +
  annotate("text", x = 3 , y= 285, label="Ward 12 on the Southwest\nSide bordering Chinatown is\ncharacterized by higher rates\nof business activity than\nits neighbors.", size=rel(6))
