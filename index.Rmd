...
wards_selected_demos <- st_join(wards.2015, selected_demos, largest=TRUE, agr = c(
 "GEOID" = "identity", "NAME" = identiy, "medianIncome" = "aggregate", "perCapitaIncome" = "aggregate"       
"population" = aggregation,              "medAgemovedInside"       "asian_pct"               "black_pct"              
"latinx_pct"              "white_pct"               "other_pct"               "below_poverty_pct"      
"childMedicaid_pct"       "childMedicare_pct"       "takeupMedicare_pct"      "takeupMedicaid_pct"     
"households_kidsSNAP_pct" "takeupSNAP_pct"          "YAmovesInChi_pct"        "movesInChi_pct"         
"immigrant_pct"           "renting_pct"             "public_transport_pct"    "asianPop"               
"belowPovertyPop"         "blackPop"                "citizenshipPop"          "femaleChildMedicaid"    
"femaleChildMedicare"     "femaleMedicaid18_64"     "femaleMedicaid65_up"     "femaleMedicare18_64"    
"femaleMedicare65_up"     "households"              "householdsKidsSNAP"      "householdsSNAP"         
"kidsHealthIns"           "latinxPop"               "maleChildMedicaid"       "maleChildMedicare"      
"maleMedicaid18_64"       "maleMedicaid65_up"       "maleMedicare18_64"       "maleMedicare65_up"      
"medicaidPop"             "medicarePop"             "mobilityPop"             "movedInside20to24"      
"movedInside25to29"       "movedInsideCounty"       "naturalized"             "noncitizen"             
"owners"                  "publicTransport"         "renterPop"               "renters"                
"transportPop"            "whitePop"                "predominant_race"        "max_pct"                
"id"                      "geometry"
))
```
...
# read in turnout and change in turnout by Chicago ward and Chciago Side
  ...
  # 2011 labels
  # geom_label_repel(data = turnoutDiff %>% group_by(YEAR, SIDE) %>% 
  #                    summarise(MEAN_TURNOUT = round(mean(TURNOUT, na.rm=TRUE),1)) %>% 
  #                    filter(YEAR == 2011),
  #           aes(x=as.factor(YEAR), y= MEAN_TURNOUT, label = paste0(MEAN_TURNOUT, "%"), 
  #               group=as.factor(SIDE), color = SIDE),
  ...
  theme_modest() + 
  theme(axis.text.x.top = element_text(size=rel(1.75), vjust = -8, face="bold", family = "Ledger"),
          axis.title.y = element_blank(),
          axis.title.x = element_blank(),
          axis.text.y = element_blank(),
          panel.grid.minor=element_blank(),
  ...
  annotate("text",  x = 1, y = 25, label = "*Underlying ward trends visualized\nbehind Side trendlines", size=rel(10), family="Ledger") + 
    annotate("text", x = 0.66, y = 68, label="Ward 19 in the Far Southwest\n Side had voter turnout of nearly\n75% in the 2011 City Council\nElection.", family="Ledger", size=rel(10), hjust=0, color=get_dt_cols("mauve"))
```
...
```{r import-school-data, message=FALSE}
...
```{r prep-for-school-plots, message=FALSE}
...
##  Quasi-Random Plot  ##
#########################
school_data %>% group_by(`SCHOOL ID (R-C-D-T-S)`, predominant_race) %>% summarise(med_LI_pct = mean(`LOW-INCOME SCHOOL %`, na.rm = TRUE)) %>%  
  ggplot(aes(predominant_race, med_LI_pct, color=factor(predominant_race))) +
      geom_quasirandom(varwidth=TRUE, size=4, show.legend = FALSE) +
  ...
  theme_modest() + 
  theme(axis.text.x.top = element_text(size=rel(2), vjust = -8, face="bold"),
