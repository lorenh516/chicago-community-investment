...
Since the Great Recession, underrepresented minority populations in Chicago have been increasingly relegated to lower income areas in the South and West of the City. Some communities have a foothold in their respective neighborhoods, while others are priced out as wealthier populations begin to move into the areas they have historically called home.
...
Among the differences in how these communities have moved over time, race is an undeniable component. There is a clar shift Latinx populations in desirable neighborhoods close to the center and north of the city to more western and southern neighborhoods.
...
The neighborhoods housing these minority communites are those with distinctly lower levels of econoic activity, while the mojority of the city's "bouncing back" after the recession happens in the Central, North, and West sides of the city, as noted in the exploration of business license issuances below.
...
There is unquestionably variation between the level of activity within each Side, however, positive variance from the norm for areas that are predominantly Black and Latinx appear to be in the few communities in which there is more of a mix of racial or ethnic populations. 
...
In light of fairly clear deinvestment in certain parts of Chicago, I hypothesized that voters in affected wards would be encouraged to vote in aldermen who would help improve their intersts. I was interested to see that the Far Southwest Side, arguably the area most impacted by deinvestment, had the highest voter turnout in both of the two most recent elections. As it turns out, the highest voter activation in this area occurred in Ward 19, the Beverly and Mt. Greenwood area, of which over 70% of the population is white, in comparison to the rest of the area, which has a white population of less than 1%, and had turnout of 35-40%. It is perhaps interesting that in a community that is largely black, another population group has the loudest voices in terms of voter input. 
...
While economc activity is definitively a factor in the a neighborhood's ability to thrive, just as important in future success is the quality of education students in an area are able to receive. Here I explore the variation in spend for students of different socioeconic backgrounds, and its potential effects.
...
Low-income percentage does not appear to have an effect on spend per student.
...
However, schools with a large proportion on average of low-income students tend to be on the lower end of the spectrum in terms of spend per student, based on the breakdown of low-income student percentages from the Illinois State Board of Education:
...
In a perhaps reassuring trend, the [newest Evidence-Based-Fuding Formula](https://cps.edu/News/Press_releases/Pages/PR1_10_5_17.aspx) used by CPS appears to have increased the number of predominantly non-white schools spending more per student.
...
```{r include=FALSE, eval=FALSE}
LI_parts <- parts_all  %>% dplyr::select(SPEND_RANGE, `Low-Income`, `Not Low-Income`) %>% gather("income_status", "value", -SPEND_RANGE)

library(ggwaffle)
waffle_data <- waffle_iron(LI_parts, aes_d(group = income_status))
