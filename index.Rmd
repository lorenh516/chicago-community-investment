...
bus_licenses %>% filter(!is.na(WARD), active==1,  activity_date >= mdy("1/1/2012")) %>%
  ...
  theme(
        ...
        axis.ticks =  element_blank(),
        axis.text.x = element_blank(),
        axis.title.y = element_text(margin=margin(0,20,0,0), angle =90, family = "Ledger"),
        plot.caption = element_text(hjust=1),
        plot.subtitle = element_text(margin = margin(t = 15))
        )
...
school_data %>% group_by(`SCHOOL ID (R-C-D-T-S)`, predominant_race) %>% 
  ...
  geom_text(
            data = school_data %>%
              ...
              distinct(`SCHOOL ID (R-C-D-T-S)`, SCHOOL_NAME, predominant_race, avg_LI_pct),
            aes(label=SCHOOL_NAME), show.legend = FALSE, hjust=1) +
  theme_modest() + 
  theme(axis.text.x.top = element_text(size=rel(1.75), vjust = -8, face="bold"),
...
school_data %>% 
  ...
  SCHOOL_NAME_CLEAN = gsub(" Elem Sch", "", SCHOOL_NAME_CLEAN),
  ...
  SCHOOL_NAME_CLEAN = paste(SCHOOL_NAME_CLEAN, " Elementary School")) %>% 
  group_by(`SCHOOL ID (R-C-D-T-S)`, SCHOOL_NAME_CLEAN, predominant_race) %>% 
  summarise(avg_LI_pct1 = mean(avg_LI_pct))
