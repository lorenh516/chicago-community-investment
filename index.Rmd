...
school_data <- left_join(
  ...
  school_data %>% group_by(`SCHOOL ID (R-C-D-T-S)`) %>% 
      summarise(
        ...
        Hispanic = mean(`SCHOOL - HISPANIC %`, na.rm=TRUE)) %>% 
...
school_data %>% group_by(`SCHOOL ID (R-C-D-T-S)`, predominant_race) %>% 
  summarise(med_LI_pct = mean(`LOW-INCOME SCHOOL %`, na.rm = TRUE),
                              school_count = n()) %>%  
...
```{r}
# sanity check number of majority white students
school_data %>% group_by(predominant_race) %>% summarise(count = n(),
                                                         median_LI = median(`LOW INCOME %`))
