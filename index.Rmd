...
dt_palettes <- list(
  `main`  = get_dt_cols("navy", "mauve", "plankton", "goldenrod"),
  `diverging` = get_dt_cols("navy", "darkteal", "honeymustard", "ocean",  "forest", "mauve"),
  `cool`  = get_dt_cols("navy","darkteal", "forest", "plankton", "ocean"),
  `desert`   = get_dt_cols("mauve", "goldenrod", "cocoa", "honeymustard"),
  `mixed` = get_dt_cols("navy",  "mauve", "goldenrod", "darkteal", "forest", "cocoa", "plankton", "honeymustard"),
...
demoConversion <- function(acs_year) {
  ...
  demo_df <- demo_df %>% mutate(
                      ...
                      ) %>% dplyr::select(GEOID, NAME, medianIncome, perCapitaIncome, population, medAgemovedInside, contains("_pct"), geometry, everything())
...
# years <- list(2012, 2013, 2014, 2015, 2016, 2017, 2018)
#
years <- list(2012, 2017)
multi_year <-
...
# # all.demos.Chi <- st_intersection(all_demos, wards.2015.acs.proj, largest = TRUE)
```
...
ggplot(st_intersection(all_demos.Chi, wards.2015, largest=TRUE) %>% 
         filter(!is.na(predominant_race))) +
  # color based on predominant race based on Census, shaded by percentage of that race
  geom_sf(aes(fill=predominant_race, alpha=medianIncome), lwd = 0) +
