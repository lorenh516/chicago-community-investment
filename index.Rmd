...
ggplot() +
  ...
  scale_fill_gradient(low = "#76BFBB", high="#2d3d61") +
  
  # outline Chicago wards over data
  geom_sf(data = wards.2015, color=get_dt_cols("mauve"), lwd = 1, fill=NA) +
...
bus_licenses %>% filter(!is.na(WARD), active==1,  activity_date >= mdy("1/1/2012")) %>%
  ...
  annotate("text", x=-0.25, y=25, label="25",size=rel(1), color=get_dt_cols("mauve"), face="bold")
...
ggplot(aes(NONWHITE_PCT, AVG_SPEND_PER_STUDENT, frame = as.integer(YEAR))) +
  ...
  theme(
        ...
        plot.subtitle = element_text(size=rel(1.25), hjust = 1),
  ...
  labs(subtitle = 'Year: {frame_time}', x = 'Non-White Student Percentage', y = 'Average Spend per Student',
       title = "Spread of Spend per Student Increases after 2018 School Funding Formula Changes") +
  transition_components(as.integer(YEAR), enter_length = as.integer(1), exit_length=as.integer(2)) +
