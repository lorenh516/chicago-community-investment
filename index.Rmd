...
```{r facet_wrap_demo_info, message=FALSE, warning=FALSE, fig.width=30, fig.height=12, eval=FALSE}

# all_demos_range <- readRDS(here::here("data", "all_demos_chi_proj.Rda"))
...
ggplot() +
  ...
  geom_sf(aes(alpha=medianIncome, color=predominant_race), lwd = 2, fill=get_dt_cols("ocean")) +
  # scale_alpha(range = c(0.15, 1)) +
  
  # outline Chicago wards over data
  geom_sf(data = wards.2015, color="black", lwd = 0.5, fill=NA) +
  scale_color_dt("diverging", reverse = TRUE, na.value=get_dt_cols("palegray")) +
  coord_sf(datum = NA) +
  theme_map_modest() + 
  theme(plot.margin = unit(c(1, 1, 1, 1), "lines"),
        plot.title = element_text(face = "bold", hjust="0.5", family = "Ledger"),
        plot.subtitle = element_text(margin = unit(c(0, 0, 1, 0), "lines")),
        plot.caption = element_text(hjust=1)) +
  facet_wrap(~ id)
  labs(alpha = "Median Income", color="Predominant Race", title="Latinx Populations in Chicago Pushed to South, West Neighborhoods", caption="Data Source: U.S. Census Bureau", subtitle = "Chicago Racial and Ethnic Group Movement\nby Census Tract Since 2012 (5 year averages)") 


  
chi_2015 <- st_intersection(wards.2015, demos_2015) %>%
ggplot() +
  # outline tracts based on predominant race based on Census, shaded by median Income for tract
  geom_sf(aes(alpha=medianIncome, color=predominant_race), lwd = 2, fill=get_dt_cols("ocean")) +
  # scale_alpha(range = c(0.15, 1)) +
  ...
  theme(plot.margin = unit(c(1, 1, 1, 1), "lines"),
        ...
        plot.subtitle = element_text(margin = unit(c(0, 0, 1, 0), "lines")),
        plot.caption = element_text(hjust=1)) +
  labs(alpha = "Median Income", color="Predominant Race")

chi_2017 <- st_intersection(wards.2015, demos_2017) %>%
ggplot() +
  # outline tracts based on predominant race based on Census, shaded by median Income for tract
  geom_sf(aes(alpha=medianIncome, color=predominant_race), lwd = 2, fill=get_dt_cols("ocean")) +
  # scale_alpha(range = c(0.15, 1)) +
...
yr_plots <- plot_grid(
  chi_2012, chi_2015, NULL, NULL, chi_2017,
  ncol = 2, nrow=3, rel_widths =  c(1, 1, 1, 1, 2), rel_widths =  c(1, 1, 1, 1, 2), align = 'h')

title <- ggdraw() + draw_label(, fontface='bold', size=15)
caption <- ggdraw() + draw_label(, size=20)
subtitle <- ggdraw() + draw_label(, size=20)

plot_grid(title, subtitle, yr_plots, subtitle, ncol=1, 
          rel_heights=c(0.01, 0.11, 1, 0.01), align='v')
  
```

```{r include=FALSE, eval=FALSE}
all_demos %>% dplyr::select(GEOID, id, medianIncome, predominant_race, max_pct, geometryspread()
```
...
bus_licenses %>% filter(!is.na(WARD), active==1,  activity_date >= mdy("1/1/2012")) %>%
  ...
  annotate("text", x = 9 , y= 745, label="The Central area is comprised\nof Ward 42, the Loop area.", size=rel(4)) +
...
school_data %>% group_by(`SCHOOL ID (R-C-D-T-S)`, predominant_race) %>% 
  ...
  ggplot(aes(predominant_race, avg_LI_pct, color=factor(predominant_race))) +
      geom_quasirandom(varwidth=TRUE, size=2, show.legend = FALSE) +
  scale_color_dt("diverging", reverse = TRUE, na.value=get_dt_cols("palegray")) +
  ...
  theme_modest() + 
  theme(axis.text.x.top = element_text(size=rel(1.15), vjust = -8, face="bold"),
        axis.text.y = element_text(size=rel(1), margin=unit(c(0,0,0,3),"lines")),
        axis.title.y = element_text(size=rel(1), angle = 90, family = "Ledger"),
        axis.title.x = element_blank()
  ...
  annotate("text", x=1.65, y = 38, label="Sutherland Elementary School*",hjust=0.5, size=rel(4), fontface="bold") + 
