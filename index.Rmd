...
bl <- bus_licenses %>% filter(activity_yr >= 2012, 
    ...
    group_by(`ACCOUNT NUMBER`, activity_date = lubridate::ymd(activity_date), `APPLICATION TYPE`, LONGITUDE, LATITUDE) %>% 
...
# pull in Chicago terrain map from Google Maps
  ggmap::ggmap(ggmap::get_googlemap(center = c(lon = -87.732125, lat = 41.83379),
    ...
    geom_point(data=bl, aes(x=LONGITUDE, y=LATITUDE, color=`APPLICATION TYPE`, group=`ACCOUNT NUMBER`), 
               alpha = 0.10, show.legend = FALSE, na.rm = TRUE) + 
    scale_color_dt() +
    ...
    labs(y="Latitude", x="Longitude", colour="Year", subtitle = 'Date: {frame_time}',
  ...
  # transition_events(start= activity_date, 
  #                   end = final_exp,
  #                   # range = NULL, 
  #                   enter_length = 2, 
  #                   exit_length = 2
  #                   ) + 
    transition_components(time=activity_date
                    ) +
  # transition_states(ymd(activity_date), 
  #                   state_length =  as.integer(max_term),
  #                   transition_length = 1) + 
  enter_grow() +
  exit_fade() +
