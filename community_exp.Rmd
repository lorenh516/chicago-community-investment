...
```{r slopegraph, fig.width=10, fig.height=15}
...
```{r avg-monthly-lic, warning=FALSE, fig.width=15, fig.height=20}
...
```{r ggmapping, message=FALSE, warning=FALSE, fig.width=10}
...
bl <- bus_licenses %>% filter(activity_yr >= 2009, active == 1, `APPLICATION TYPE`%in% c('ISSUE', 'RENEW')) %>% 
  ...
  ggmap::ggmap(ggmap::get_googlemap(center = c(lon = -87.732125, lat = 41.83379),
    ...
    theme(
      ...
      # panel.grid.major.x = element_blank(),
      # panel.grid.minor.x = element_blank(),
      plot.subtitle = element_text(size = 10, margin=margin(t=20)),
