...
dt_palettes <- list(
  `main`  = get_dt_cols("teal", "lavendar", "honeymustard"),
  `cool`  = get_dt_cols("darkteal", "teal", "seafoam",
                          "lavender"),
...
```{r}
min(wards.2015.acs.proj$long)
median(wards.2015.acs.proj$long)
max(wards.2015.acs.proj$long)
```



```{r facet_wrap_demo_info, message=FALSE, warning=FALSE, fig.width=10}
chiCentralLongitude <- -87.68008

ggplot(st_intersection(all_demos, wards.2015.acs.proj) %>% filter(!is.na(max_pct))) +
  ...
  geom_text_repel(data=wards.2015.acs.proj %>% filter(ward %in% c(4, 9, 10, 13, 14, 22, 18, 33, 34), long < chiCentralLongitude), 
...
bus_licenses %>% filter(!is.na(WARD), active==1) %>%
  ...
  geom_label_repel(aes(label=as.factor(WARD))) + 
