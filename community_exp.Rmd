...
demoConversion <- function(acs_year) {
  ...
  demo_df <- demo_df %>% mutate(
    ...
    demo_df <- left_join(demo_df, demo_df %>% group_by(GEOID, NAME) %>% 
                           select(GEOID, NAME, geometry, black_pct, latinx_pct, asian_pct, white_pct, other_pct) %>%
                           ...
                           dplyr::select(GEOID, NAME, predominant_race), by=c("GEOID", "NAME"))
    return(demo_df)
}
```
...
multi_year <-
  map(years ~ demoConversion(year = .x)) %>%
  map2(years, ~ mutate(.x, id = .y))

all_demos <- reduce(multi_year, rbind)

ggplot(all_demos) +
  geom_sf(aes(fill = medianIncome, color=predominant_race)) +
  coord_sf(datum = NA) +
  facet_wrap(~ id)
```
