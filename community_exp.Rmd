...
bus_licenses %>% filter(activity_yr >= 2009, active == 1, `APPLICATION TYPE`%in% c('ISSUE', 'RENEW')) %>% 
  ...
  geom_text(data = wards2015_spacial_centroids.df, aes(long,lat, group=group, label=ward, hjust = .50, vjust = .50)) + 
  coord_equal(ratio=1) + 
...
school_cols_to_import <- c("Avg Class Size - High School", "Avg Class Size – All Grades", "Avg Teaching Exp", "Avg Teaching Exp - High Poverty", "Avg Teaching Exp - Low Poverty", "Bachelor Degree	Bachelor Degree - High Poverty", "Bachelor Degree - Low Poverty","Masters Degree	Masters Degree - High Poverty", "Masters Degree - Low Poverty", "Pupil Teacher Ratio - Elementary", "Pupil Teacher Ratio - High School", "Pupil Certified Staff Ratio", "Pupil Admin Ratio", "Teacher Avg Salary", "Admin Avg Salary", "Teacher Retention Rate")
```
