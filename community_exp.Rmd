...
wardsPre2015 <- wardsPre2015 %>% mutate(ctr = sf::st_transform(wards.2015, 29101) %>% 
  st_centroid() %>% 
...
demos_17 <- getDemos(acs_year=2017)
```
...
demoConversion <- function(acs_year) {
  ...
  demo_df <- demo_df %>% mutate(
    ...
    demo_df <- st_join(demo_df, demo_df %>% group_by(GEOID, NAME) %>% 
                           dplyr::select(GEOID, NAME, geometry, black_pct, latinx_pct, asian_pct, white_pct, other_pct) %>%
                           gather(group_name, pct, -GEOID, -NAME, -geometry) %>% 
                           ...
                           dplyr::select(GEOID, NAME, predominant_race)
                       # , by=c("GEOID", "NAME")
                       )
    return(demo_df)
}
...
ggplot(st_intersection(all_demos, st_transform(wards.2015, crs = st_crs(all_demos)))) +
  geom_sf(aes(fill=predominant_race), lwd = 0) +
  geom_sf(data = wards.2015, color="black", fill=NA) +
  geom_sf_label(data = wards.2015, aes(label=ward)) +
  coord_sf(crs = st_crs(all_demos), datum = NA) +
  theme_minimal() + 
  ...
  facet_wrap( ~ id) + labs(
    title = "Predoinantly Latinx, Black Populations Being Edged out of Waterfront Neighborhoods",
