...
diverging_teal[5] <- "#C6C3FF"
diverging_teal[6] <- "#9AEADE"
diverging_teal[3] <- "#B5B5B5"
diverging_teal[2] <- "#E8B45B"
diverging_teal[1] <- "#E8B45B"
names(diverging_teal) <-  c("cocoa", "honeymustard", "darkgray", "sand", "lavender", "seafoam", "palegray", "teal", "darkteal")
```

```{r}
diverging_teal["darkgray"]
```
...
dt_palettes <- list(
  ...
  `desert`   = get_dt_cols("chai", "sand", "honeymustard", "cocoa"),
  `mixed` = get_dt_cols("cocoa", "honeymustard", "sand", "lavender", "seafoam", "teal", "darkteal"),
...
chiCentralLongitude <- median(wards.2015.acs.proj$long)

ggplot(st_intersection(all_demos, wards.2015.acs.proj) %>% filter(!is.na(predominant_race))) +
  ...
  geom_sf(data = wards.2015.acs.proj, color="black", fill=NA) +
    geom_text_repel(data=wards.2015.acs.proj %>% filter(ward %in% c(10, 18)), 
                  ...
                  direction = "both",
                  hjust=0) +
  geom_text_repel(data=wards.2015.acs.proj %>% filter(ward %in% c(9, 13, 14, 22, 33, 34)), 
                  aes(x=long, y=lat, label=ward),
                  nudge_x = -.35,
                  segment.size = 0.5,
                  ...
                  direction = "both",
                  hjust=0) +
  geom_text_repel(data = wards.2015.acs.proj %>% filter(ward %in% c(4, 11)), 
                  aes(x=long, y=lat, label=ward),
                  nudge_x = .15,
                  segment.size = 0.5,
                  segment.color = get_dt_cols("cocoa"),
  ...
  facet_wrap( ~ id) + labs(
    title = "Latinx Populations Edged out of Central, Waterfront Neighborhoods",
...
```{r post-2012-lic, warning=FALSE, fig.width=10, include=FALSE}
...
```{r save-post-2012-lic, include=FALSE}
...
bus_licenses %>% filter(!is.na(WARD), active==1) %>%
  ...
  scale_fill_dt("mixed") +
  geom_label_repel(aes(x=reorder(as.factor(WARD), business_count, FUN = median), y=median(business_count), label=as.factor(WARD))) + 
...
```{r}
turnout <- merge(voterTurnout, wardSides, by="WARD") %>% filter(PRECINCT=="Total") %>% 
...
```{r fig.width=8}
  ggplot(turnout, aes(x=as.factor(YEAR), y=TURNOUT, group=as.factor(SIDE))) + 
  geom_line(size=1.5, alpha=0.75, color="grey", show.legend = FALSE) +
  geom_line(data = filter(turnout, (DIFFERENCE > -0)||(is.na(DIFFERENCE))), aes(colour=SIDE), size=1.5, show.legend=FALSE) +
  # geom_label(data = turnoutDiff %>% group_by(YEAR, SIDE) %>% summarise(MEAN_TURNOUT = round(mean(TURNOUT, na.rm=TRUE),1)),
  #           aes(label = paste0(MEAN_TURNOUT, "%"), group=as.factor(SIDE)), 
  #           color = "darkgray",
  #           label.padding = unit(0.05, "lines"),  
  #           label.size = 0.0,
  #           fontface = "bold",
  #           size = 4) + 
  # geom_line(aes(group=as.factor(WARD), color=SIDE), size=0.5, alpha=0.25, show.legend = FALSE) +
  geom_label_repel(data = turnout %>% filter(YEAR == 2011),
            ...
            size = 5,
            show.legend = FALSE) +
  geom_label_repel(data = turnout %>% filter(YEAR == 2015),
            aes(label = paste0(SIDE), color = SIDE),
            hjust = "right",
            nudge_x = .25,
            force = 5,
  ...
  #         # panel.grid.major.y=element_blank(),
          panel.grid.major.x=element_blank()) +
  labs(title="Highest Voter Turnout Could be Affecting Alderman Motivation \nto Fight Deinvestment", 
