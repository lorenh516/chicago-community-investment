...
subtitle_1 <- grid::textGrob(expression("New Business beyond " * phantom(bold("Loop")) * "Concentrated in" * phantom(bold("Pilsen")) * " ," * phantom(bold("East Garfield Park")) * " ," *  phantom(bold("Humbolt Park")) ),
                 x = 0.5, y = 2.1, gp = grid::gpar(col = "black"))

subtitle_2 <- grid::textGrob(expression("New Business beyond " * phantom(bold("Loop")) * "Concentrated in" * phantom(bold("Pilsen")) * " ," * phantom(bold("East Garfield Park")) * " ," *  phantom(bold("Humbolt Park")) ),
                 x = 0.5, y = 2.1, gp = grid::gpar(col = "#f67770"))

subtitle_3 <- grid::textGrob(expression("New Business beyond " * phantom(bold("Loop")) * "Concentrated in" * phantom(bold("Pilsen")) * " ," * phantom(bold("East Garfield Park")) * " ," *  phantom(bold("Humbolt Park")) ),
                 x = 0.5, y = 2.1, gp = grid::gpar(col = "#1bb840"))
      
subtitle_4 <- grid::textGrob(expression("New Business beyond " * phantom(bold("Loop")) * "Concentrated in" * phantom(bold("Pilsen")) * " ," * phantom(bold("East Garfield Park")) * " ," *  phantom(bold("Humbolt Park")) ),
                 x = 0.5, y = 2.1, gp = grid::gpar(col = "#649efc"))

bus_licenses %>% filter(activity_date >= "2013-1-1", !is.na(WARD), active==1) %>%
  group_by(activity_wk, WARD) %>% summarise(business_count = n()) %>% 
  ggplot() + geom_line(aes(x=activity_wk, y=business_count, colour=as.factor(WARD)), alpha=0.6) +
  gghighlight(max(business_count), max_highlight = 3L) + 
  labs(y="Business Count", x="Activity Week", colour="Chicago Council Ward", title="Over Five Years of Gentrification Over Rehabilitation", caption="Data Source: Chicago Open Data Portal") +
...
```{r}
ggsave("location_over_googlemap.pdf", width = 36, height = 23, units = "cm")
```
...
```{r}
voter_turnout15 %>% filter(PRECINCT=="Total") %>% 
  ggplot(aes(y=as.factor(WARD), x = TURNOUT.2015)) + geom_text(aes(label=WARD), size=3) + labs(x="Turnout %", y="City Council Ward") + theme_minimal() + theme()
```
 ...
 merge(x=voter_turnout11, y=voter_turnout15) %>% filter(PRECINCT=="Total") %>% 
  ggplot(aes(x=TURNOUT.2011, xend=TURNOUT.2015, y=WARD, group=WARD)) + 
        geom_dumbbell(color="#e3e2e1", 
                      ...
                      colour_xend="#43a2ca") + 
        geom_vline(aes(
          xintercept = unlist(voter_turnout15 %>%  filter(PRECINCT == "Total") %>% dplyr::summarise(m = mean(TURNOUT.2015, na.rm=TRUE)))),
          color = "#43a2ca", size=1, alpha=0.75, linetype="dashed") +
         geom_vline(aes(
           xintercept = unlist(voter_turnout11 %>% filter(PRECINCT == "Total") %>%  dplyr::summarise(m = mean(TURNOUT.2011, na.rm=TRUE)))),
      ...
      theme(plot.title = element_text(size = 15, face = "bold"),
            ...
            legend.position="top",
            panel.border=element_blank()) + 
      annotate("text", x=5 + (unlist(voter_turnout11 %>% filter(PRECINCT == "Total") %>% dplyr::summarise(m = mean(TURNOUT.2011, na.rm=TRUE)))),
            y=30.5, color = "#a8ddb5", label="2011 Average",size=3) +
      annotate("text", x=-5 + (unlist(voter_turnout15 %>% filter(PRECINCT == "Total") %>% dplyr::summarise(m = mean(TURNOUT.2011, na.rm=TRUE)))),
            y=47, color = "#43a2ca", label="2011 Average", size = 3) +
      theme_minimal()
