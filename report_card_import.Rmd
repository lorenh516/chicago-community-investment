...
for (i in 1:length(h)) {
  if (is.character(h[[i]])) { 
    if (str_detect(h[[i]], "(20th.*)([0-9]{4})[-]([0-9]{4} )")) {
  h[i] <- unlist(strsplit(h[[i]], "(20th.*)([0-9]{4})[-]([0-9]{4} )"))[2]
    }
  }
}
```
...
```{r}
multiHeader(filename=here::here("data", "rc_v2", "Enrollment_Info_SpEd_Race", "FY12_Racial_Ethnic_Survey.xls"), sheet="Educational Units", n_max=2)
```
