...
ALL_SCHOOL_INFO <-  ALL_SCHOOL_INFO %>% mutate(
                           ...
                           Long_Name = ifelse(!is.na(Long_Name), Long_Name,
                                       ...
                                       ifelse(!is.na(`Name of School`),`Name of School`,
                                              Long_Name))),
                           One_Year_Dropout_Rate_Year_2_Pct = ifelse(is.na(One_Year_Dropout_Rate_Year_2_Pct) & YEAR == 2013,
                                                                     `One-Year DropOut Rate Percentage 2013`,
                                                                     One_Year_Dropout_Rate_Year_2_Pct),
                           Student_Attendance_Year_2_Pct = ifelse(is.na(Student_Attendance_Year_2_Pct) & YEAR == 2013,
                                                                  `Student Attendance Percentage 2013`,
                                                                  Student_Attendance_Year_2_Pct),
                           Teacher_Attendance_Year_2_Pct = ifelse(is.na(Teacher_Attendance_Year_2_Pct) & YEAR == 2013,
...
all_report_cards %>% distinct(`SCHOOL ID (R-C-D-T-S)`, `SCHOOL NAME`, `SCHOOL TYPE NAME`)  %>%
 ...
 ALL_SCHOOL_INFO %>% distinct(School_ID, Long_Name, Address, Latitude, Longitude, ZIP) %>%
   write_csv("ALL_SCHOOL_LIST.csv")
                               anti_join(all_report_cards, by=c("Long_Name" = "SCHOOL NAME")) %>% 
          anti_join(all_budgets, by=c("Long_Name" = "Long_Name"))
 
  ALL_SCHOOL_INFO %>% distinct(School_ID, Long_Name, Address) %>%
                               right_join(all_budgets, by=c("Long_Name" = "Long_Name"))
 
  ALL_SCHOOL_INFO %>% distinct(School_ID, `School ID`, Long_Name, `School Name`, Address, `Street Address`) %>%
                               semi_join(all_report_cards, by=c("Long_Name" = "SCHOOL NAME"))


all_report_cards %>% semi_join(ALL_SCHOOL_INFO %>% 
                                 distinct(School_ID, `School ID`, Long_Name, `School Name`, Address, `Street Address`),
                               by=c("SCHOOL NAME" = "Long_Name")) %>% unique()
```
...
school_data <- school_data %>% 
  mutate(AVG_SPEND_PER_STUDENT = as.numeric( 
  ...
  dplyr::select(YEAR, PREV_YR, TWO_YR_PRIOR, School_ID, WARD, NAME_WARD_JOIN, 
                CY_BUDGET = `Proposed Budget`, SCHOOL_TOTAL_ENROLLMENT =  `SCHOOL TOTAL ENROLLMENT`,
...
school_data %>% 
  ...
  geom_line(aes(x=AVG_SPEND_PER_STUDENT, y=YEAR)) +
  geom_point(aes(x=AVG_SPEND_PER_STUDENT, y=YEAR, color=SIDE, size=SCHOOL_TOTAL_ENROLLMENT)) +
...
wards_w_all_demos <- st_join(wards.2015, all_demos)
  
  group_by(id, SIDE) %>% summarise(mean_takeup = mean(takeupSNAP_pct, na.rm=TRUE))
...
school_data %>% group_by(YEAR, SIDE) %>% 
  ...
  geom_line(aes(group=as.factor(SIDE), colour=SIDE)) +
...
school_data <- school_data %>%
  mutate(SCHOOL_TOTAL_ENROLLMENT = as.numeric(gsub(",","",SCHOOL_TOTAL_ENROLLMENT)),
         YEAR = as.numeric(YEAR),
         PREV_YR = as.numeric(PREV_YR),
         TWO_YR_PRIOR = as.numeric(TWO_YR_PRIOR))
# cut(SCHOOL_TOTAL_ENROLLMENT, breaks=5, ordered_result = TRUE)
school_data %>% 
  ggplot(aes(x=AVG_SPEND_PER_STUDENT, y=SCHOOL_TOTAL_ENROLLMENT, group=as.factor(SIDE))) +
  geom_point(aes(color=SIDE), position="jitter",size=8) +
    scale_color_dt(na.value=get_dt_cols("palegray")) +
