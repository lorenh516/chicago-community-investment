...
```{r}
multiHeader <- function(filename, sheet=NA, col_names=TRUE, na="..", n_max=1) {
  ...
  for (i in length(column_labels)) {
    column_labels[i] <- gsub("\n", "\\s",column_labels[i]) 
  }
  
  # pull them into one character vector, add country.
  headers = c("YEAR", unname(unlist(column_labels[1,])))
...
```{r}
multiHeader(filename=here::here("data", "rc_v2", "Enrollment_Info_SpEd_Race", "FY14_Racial_Ethnic_Survey.xls"), sheet="Educational Units", n_max=2)
```
