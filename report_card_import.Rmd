...
```{r}
read_csv(here::here("data", "Chicago_Public_Schools_-_School_Profile_Information_SY1617.csv"), col_names = TRUE)
```
...
```{r}
all_budgets <- left_join(all_budgets, schoolsByWard, by = c("unitNum" = "Finance_ID")) %>% 
  select( contains('_ID'), contains("long_name"),contains("address"), one_of(colnames(all_budgets)), contains("latitude"), contains("longitude"))
```
