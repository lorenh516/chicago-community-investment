...
inner_join(all_budgets, 
  ...
  # filter(!is.na(School_ID.y) & is.na(School_ID.x))

%>% 
  dplyr::select( contains('_ID'), contains("long_name"),contains("address"), 
                 contains("latitude"), contains("longitude"), everything()) 
...
school_data <- school_data %>% 
  mutate(`SCHOOL TYPE NAME` = str_trim(`SCHOOL TYPE NAME`))
...
school_data %>%        
  ggplot(aes(`LOW-INCOME SCHOOL %`, predominant_race)) +
      geom_beeswarm(cex = 1.2, priority="density", groupOnX=FALSE, aes(color=predominant_race)) +
  scale_color_dt("diverging", reverse = TRUE, na.value=get_dt_cols("palegray")) +
  # scale_x_continuous(breaks=c(0, 25, 50, 75, 100)) +
  theme_modest() + 
  labs(title = "Greater Low-Income Percentage In Non-White Schools", fontface='bold')
...
school_data %>% group_by(`SCHOOL ID (R-C-D-T-S)`, predominant_race) %>% summarise(med_LI_pct = mean(`LOW-INCOME SCHOOL %`, na.rm = TRUE)) %>%  
  ...
  theme(axis.text.x.top = element_text(size=rel(2), vjust = -8, face="bold"),
        axis.text.y = element_text(size=rel(1.5), margin=unit(c(0,0,0,3),"lines")),
  ...
  labs(title = "Chicago's public schools display enormous disparity in percentage of low-income students\nbetween majority non-white and majority white schools", 
...
beg_mid_end <- all_demos %>% filter(YEAR %in% c(2015, 2012, 2017))



  ggplot(st_join(wards.2015, beg_mid_end, largest=TRUE) ) + 
...
school_data %>% 
  ...
  summarise(median_class_size = median(`OVERALL AVERAGE CLASS SIZE - SCHOOL`),
            num_schools = n()) %>%
  ggplot() +
