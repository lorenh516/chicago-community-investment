...
ggplot(aes(NONWHITE_PCT, AVG_SPEND_PER_STUDENT, frame = as.integer(YEAR))) +
  geom_point(alpha = 0.7, show.legend = FALSE, aes(size = `OVERALL AVERAGE CLASS SIZE - SCHOOL`,
  ...
  labs(subtitle = 'Year: {frame_time}', x = 'Non-White Percentage', y = 'Average Spend per Student',
...
bl <- bus_licenses %>% filter(activity_date >= 2012, active == 1, `APPLICATION TYPE`%in% c('ISSUE', 'RENEW')) %>% 
    distinct(`LICENSE ID`, activity_date, `APPLICATION TYPE`, LONGITUDE, LATITUDE) 
...
# pull in Chicago terrain map from Google Maps
  ...
  transition_time(as.integer(activity_date)) +
  ease_aes('sine-in-out')
```
